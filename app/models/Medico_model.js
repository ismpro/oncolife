...
class Medico {
    ...
    static async getOneById(id) {
        if (id && !isNaN(id) && Number.isSafeInteger(id)) {
            try {
                let query = await pool.query(`SELECT med_id"id", med_licen"licen", med_espec"espec", med_espec"pes_id" FROM medico WHERE med_id = ${id}`);
    ...
    static async getAll() {
        let medicos = []
        try {
            let query = await pool.query(`SELECT med_id"id", med_licen"licen", med_espec"espec", med_espec"pes_id" FROM medico`);
