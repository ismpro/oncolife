...
class SessaoPaciente {
    ...
    constructor(obj) {
        ...
        this.date = obj.date //ses_pac_date
    ...
    async create() {
        let info = await pool.query(`INSERT INTO sessaopaciente (ses_pac_freq, ses_pac_date, ses_pac_hora, ses_pac_pac_id, ses_pac_ses_id) VALUES (${this.frequencia}, '${this.date}', '${this.hora + ':00'}', ${this.paciente}, ${this.sessao})`);
    ...
    static async getAllByPacId(id) {
        let sessaopacientes = []
        try {
            let query = await pool.query(`SELECT ses_pac_id"id", ses_pac_date"date", ses_pac_freq"frequencia", ses_pac_hora"hora", ses_pac_pac_id"pac_id", ses_pac_ses_id"ses_id" FROM sessaopaciente WHERE ses_pac_pac_id = ${id}`);
    ...
    static async getOneById(id) {
        if (id && !isNaN(id) && Number.isSafeInteger(id)) {
            try {
                let query = await pool.query(`SELECT ses_pac_id"id", ses_pac_date"date",ses_pac_freq"frequencia", ses_pac_hora"hora", ses_pac_pac_id"pac_id", ses_pac_ses_id"ses_id" FROM sessaopaciente WHERE ses_pac_id = ${id}`);
    ...
    static async getAll() {
        let sessaopacientes = []
        try {
            let query = await pool.query(`SELECT ses_pac_id"id", ses_pac_date"date", ses_pac_freq"frequencia", ses_pac_hora"hora", ses_pac_pac_id"pac_id", ses_pac_ses_id"ses_id" FROM sessaopaciente`);
            for (const element of query) {
                let data = await Promise.all([Sessao.getOneById(element.ses_id), Paciente.getOneById(element.pac_id)])
                let paciente = data[1]
                let sessao = data[0]
